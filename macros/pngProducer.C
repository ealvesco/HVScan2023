...
void DrawingEff(const char* ,const char* , int, Double_t *,Double_t *, Double_t *,Double_t *, Double_t *, const char*, const char*);
void DrawingCls(const char* ,const char* , int, Double_t *,Double_t *, Double_t *,Double_t *, Double_t *, const char*, const char*);
...
std::vector< std::pair<std::string,std::string> > dictionary(const char* subd){
  ...
  Double_t id;
  std::string name;
  if (strncmp(subd,"endcap",6)==0)f.open("../data/detIdEndCapMiniScan2022.txt");
  else f.open("../data/detIdBarrelMiniScan2022.txt");
...
//  c1->SetGridy(1); 
 ...
 hveff->GetXaxis()->SetTitleOffset(0.5);
 hveff->GetYaxis()->SetTitleSize(0.05);
 hveff->GetYaxis()->SetTitleOffset(0.5);
 hveff->Draw("AP");
 ...
 c1->SaveAs(("../results/"+name_+"_EFFvsHV.png").c_str());
 c1->SaveAs(("../results/"+name_+"_EFFvsHV.C").c_str());
 c1->Clear(); 
 return; 
}
 
 void DrawingCls(const char* subdetect,const char* chamber, int points, Double_t *hv,Double_t *hverr, Double_t *cls, Double_t *clserr, Double_t* fitparamCls,                  const char* id, const char* name){
 gROOT->Reset();
 if (points ==0.) {std::cout << "N O  D A T A "; return;} 
 std::string chamber_ = chamber;
 std::string id_ = id;
...
//gSystem->mkdir(("../results/"+chamber_).c_str());
 c2->SaveAs(("../results/"+name_+"_CLSvsHV.png").c_str());
...
void pngProducer(const char* subdetect){
   ...
   {      
      ...
      DrawingEff(subdetect, c, RUN, HV, HVerr, EFF, ERR, FitResEff, (id_).c_str(), (name_).c_str());
	  DrawingCls(subdetect, c, RUN, HV, HVerr, CLS, CLSerr, FitResCls, (id_).c_str(), (name_).c_str());
    }
  exit(0); 
}
